  ...
  async function pFunc(resolve, reject) {
    RECIPE_URLS.forEach(async (url, index) => {
      ...
      try {
        ...
        //            resolve() method.
        if (fetchedR.length == RECIPE_URLS.length) {
          saveRecipesToStorage(fetchedR);
          resolve(fetchedR);
        }
      } catch (error) {
